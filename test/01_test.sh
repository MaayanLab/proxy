...
echo "Proxy catch, internal add slash redirect"
assert "$(curl -so/dev/null ${TEST_BASE}/01 -w '%{http_code}')" "200" "HTTP code"
assert "$(curl -s ${TEST_BASE}/01)" "01" "Proper content"
...
echo "Proxy catch, internal add slash multi-redirect regex"
assert "$(curl -so/dev/null ${TEST_BASE}/01 -w '%{http_code}')" "200" "HTTP code"
assert "$(curl -s ${TEST_BASE}/01/02/03/02/)" "02" "Proper content"
